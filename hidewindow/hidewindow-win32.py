...
def isfocused():
    ...
    window_id = win32gui.GetWindowText(hwnd)
...
def update_source(calldata=None): 
    ...
    source_wid = S.obs_data_get_string(props,"window").split(':')[0]
    # properties = S.obs_get_source_properties(source.id);
    print("updated source: ",source_wid)
    # print(properties);
    print(S.obs_data_get_string(props,"window"))
    # print(last_seen)
    S.obs_source_release(source)
...
def script_defaults(settings):
    S.obs_data_set_default_int(settings, "interval", 5000)
